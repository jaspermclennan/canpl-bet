...
def main() -> None:
    ...
    # CRITICAL: Align with the new positional columns
    required = {"playerId", "playerName", "season", "Minutes", "TotalShrunk", "PercentileRank"}
    ...
    df = df.dropna(subset=["season", "TotalShrunk"])
    ...
    score_cols = ["TotalShrunk", "PercentileRank"]
    if "AttackShrunk" in df.columns: score_cols.append("AttackShrunk")
    if "DefenseShrunk" in df.columns: score_cols.append("DefenseShrunk")
    ...
    if "AttackShrunk" in df.columns:
        agg_dict["sum_weighted_attack"] = ("weighted_AttackShrunk", "sum")
    if "DefenseShrunk" in df.columns:
        agg_dict["sum_weighted_defense"] = ("weighted_DefenseShrunk", "sum")

    career = df.groupby(["playerId", "playerName"]).agg(**agg_dict).reset_index()
    career = career[career["sum_w"] > 0].copy()
    
    # 4. Final Career Averaging
    ...
    if "sum_weighted_attack" in career.columns:
        career["AttackShrunk"] = (career["sum_weighted_attack"] / career["sum_w"]).round(3)
    if "sum_weighted_defense" in career.columns:
        career["DefenseShrunk"] = (career["sum_weighted_defense"] / career["sum_w"]).round(3)

    # 5. Logical Column Organization
    keep_cols = [
        "playerId", "playerName", "TotalMinutes", "SeasonsPlayed", 
        "TotalShrunk", "PercentileRank", "PeakTotal", "PeakPercentile"
    ]
    if "AttackShrunk" in career.columns: 
        keep_cols.extend(["AttackShrunk", "DefenseShrunk"])
