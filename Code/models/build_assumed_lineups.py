...
def run_validation(df: pd.DataFrame):
    ...
    if mins_fail.empty:
        print("MINUTE CHECK PASSED: All matches total 990m.")
    else:
        print(f"MINUTE ERROR: {len(mins_fail)} teams failed the 990m target.")

    if not roster_fail.empty:
        print(f"⚠️ DATA GAP: {len(roster_fail)} teams have < 11 players in database.")
        print("   (Expected for teams with missing rows like Edmonton 2022)")
    else:
...
def main() -> None:
    ...
    for _, m in matches.iterrows():
        # Force native types for comparison
        s_val = int(m["Season"])
        d_val = str(m["Date"])
        ...
        for team_col in ["Hometeam", "Awayteam"]:
            ...
            if tp.empty:
                out_rows.append({
                    ...
                    "playerName": "", "expected_minutes": 0.0, "source": "MISSING"
                })
                continue
    ...
    print(f"Process Complete! Saved to: {OUT_FILE}")
    run_validation(out)
